...
public class LazyString {
    ...
    public LazyString(String source, int start, int end) {
        ...
        /* ************************** Вопрос № 1: **********************************
        Филипп, я так понимаю циклом же подсчитывать hash? В гонке за асимптотику боюсь его применять.
        Но тот факт, что hash подсчитывается от end - start, величина как бы фиксированная,
        то верно ли считать выполнение за время О(1) ???
        ****************************************************************************
         */

        String subString = source.substring(start, end);
        for (int i = 0; i < subString.length(); i++) {
            hash += subString.charAt(i); // хэш == 0 при создании экземпляра класса LazyString
        }
    }
    ...
    public LazyString shiftRight() {
        ...
        // ****************** Ох уж эта сила ключевого слова this *************************
        shifted.hash = this.hash - this.source.charAt(this.start) + this.source.charAt(end);
